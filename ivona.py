...
voices = ['sam', 'mary', 'mike', 'anna', 'lili', 'daniel', 'ivonademo', 'jacek', 'ewa', 'jennifer', 'carmen',
          'eric', 'jan', 'maja', 'brian', 'amy', 'joey', 'kendra', 'kimberly', 'emma', 'chipmunk', 'hans',
          'marlene', 'enrique', 'conchita', 'miguel', 'penelope', 'mathieu', 'celine', 'salli', 'ivy',
          'gerainten', 'geraintcy', 'gwynethen', 'gwynethcy', 'nicole', 'agnieszka', 'giorgio', 'chantal',
          'ricardo', 'vitoria', 'lotte', 'naja', 'karl', 'dora', 'russell', 'carla', 'ruben', 'mads', 'tatyana',
          'astrid', 'cristiano', 'filiz', 'raveena', 'krzysztof', 'zosia', 'dave', 'steven', 'ludoviko']


def create_tts(text, voice, filename) -> str:
    arg = ''
    if voice == 'ivonademo':
        arg = text.split(' ', 1)[0]
    ...
    if voice in ('anna', 'lili'):
        arg = ' -16'
    if voice == 'ivonademo':
        os.system('bin/ivonacl -f /tmp/' + filename + ' -l bin/libvoice_pl_jl16demo.so --dur ' + arg +
                  ' cache/ivonademo/' + filename + '.wav')
    else:
        os.system('bin/generate_sapi5.exe -i /tmp/' + filename + ' -n ' + voice + arg + ' -o cache/' + voice + '/' +
                  filename + '.wav 2>/dev/null')
...
def find_sound(text, voice):
    # Lili is a Chinese voice and doesn't need conversion to Latin
    if voice == 'lili':
...
async def clean_cache() -> None:
    ...
    for folder in voices:
        ...
        for audiofile in os.listdir('cache/' + folder):
            if now or (time.time() - os.path.getatime('cache/' + folder + '/' + audiofile) > 604800):
